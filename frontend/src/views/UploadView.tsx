...
export function UploadView() {
  ...
  const [loading, setLoading] = useState<"resume" | "jd" | null>(null);
  ...
  async function handleJD(event: FormEvent<HTMLFormElement>) {
    ...
    <div className="max-w-5xl">
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        <motion.label
          ...
          className="flex min-h-72 cursor-pointer flex-col justify-between rounded-md border border-dashed border-slate-300 bg-white p-6 shadow-soft transition hover:border-mint"
        >
          <div>
          ...
          <div>
            <p className="mb-3 truncate text-sm font-medium text-slate-700">
              {resumeFileName || "No resume uploaded yet"}
            </p>
            <span className="inline-flex rounded-md bg-mint px-4 py-2.5 text-sm font-semibold text-white">
        ...
        <form onSubmit={handleJD} className="rounded-md border border-slate-200 bg-white p-6 shadow-soft">
          <h3 className="text-xl font-semibold text-ink">Job Description</h3>
          <textarea
            value={jdDraft}
            onChange={(event) => setJdDraft(event.target.value)}
            className="mt-4 h-52 w-full resize-none rounded-md border border-slate-200 p-3 text-sm leading-6 outline-none transition focus:border-mint focus:ring-2 focus:ring-mint/20"
